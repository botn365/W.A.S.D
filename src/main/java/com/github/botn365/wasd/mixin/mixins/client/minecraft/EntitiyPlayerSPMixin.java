...
public abstract class EntitiyPlayerSPMixin extends AbstractClientPlayer {
    ...
    public boolean cancelFleigtMovment(boolean original) {
        if (original && WASDInit.isInit() && this.mc.currentScreen == null && Settings.fleightEnabled) {
            val deviceID = WASDInit.getDeviceID();
            val f = getResponseCurve(UP).translate(wootingAnalogReadAnalog(this.mc.gameSettings.keyBindJump.getKeyCode()));
            this.motionY += f * 0.15;
